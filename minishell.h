...
/* структура (на односвязанных листах) для хранения и работы
с переменными окружения ключ - переменная значение - ее состояние */
...
/* структура (на односвязанных листах) в которую записывается результат
работы лексера после разбивки принятой строки на отдельные части
(разбивка по " ", ";", "|", а также символы "\'" и "\"" разбиваются
одним блоком, а "\\" сохраняет следующий символ), при этом символы разделителя
так же сохраняются в отдельных листах */
...
/*функция запускается при старте прграммы и сохраняет переменные окружения
в листы для последующей работы с ними.
...
/*функция проверяет если в чанке последним $ и следующий чанк существует
и он не разделитель не пробел не редирект, то знак $ не печатается*/
...
/* функция работы с файловыми дискрипоторами
в дочернем процессе (последний пайп)*/
...
/* функция из библиотеки Readline. Прототип добавлен для корректной
сборки */
